import {type NodeId, ROMANIA} from './romania.ts';
import {type SearchResult, type Step, reconstructPath} from './search.ts';
...
export function biucs(start: NodeId, goal: NodeId): SearchResult {
  ...
  while(!commonDest){
    ...
    for(const edgeWithCost of pendingVisitBackwardPaths)
      frontierNodes.add(edgeWithCost.to);
    const visitedNodes: NodeId[] = result.steps.at(-1) ? Array.from(result.steps.at(-1).visited) : [];
