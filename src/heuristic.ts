...
// Heuristic-private geometry (4000x2250 space) — decoupled from romania.ts's
// 600x450 map coords so the friction/arc math stays untouched regardless of
// map redesigns. Values copied verbatim from origin/arc-heuristic:src/romania.ts.
// Edge/km connectivity is still read from ROMANIA (identical across branches).
const HCOORDS: Record<string, { x: number; y: number }> = {
  Oradea: { x: 985, y: 446 },
  Zerind: { x: 865, y: 658 },
  Arad: { x: 773, y: 874 },
...
function edgeGeometry(): EdgeGeom {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const ca = HCOORDS[a], cb = HCOORDS[b]
    x1[i] = ca.x; y1[i] = ca.y
...
const BX = HCOORDS['Bucharest'].x
const BY = HCOORDS['Bucharest'].y
...
export function arcGeometry(start: string, goal: string) {
  const ca = HCOORDS[start], cb = HCOORDS[goal]
  const dx = cb.x - ca.x, dy = cb.y - ca.y
  const chord = Math.hypot(dx, dy)
